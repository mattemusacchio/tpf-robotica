"""Parte C rosbag validation stack.

This launch is intentionally open-loop: it validates red-cone perception and
goal-to-planner integration over the recorded bag, without running the path
follower or navigation state machine.  A rosbag cannot respond to /cmd_vel, so
closed-loop navigation would look misleading in RViz.
